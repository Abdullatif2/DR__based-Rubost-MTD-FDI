...
test_dataset = PowerDataDataset(csv_file, start_row=25000, num_rows=ac_opf_no_ac)    # First 25,000 rows for training
dataloader = DataLoader(test_dataset, batch_size=ac_opf_no_ac, shuffle=False)
...
for i in range(ac_opf_no_ac):
    ...
    active_power = batch['active_power'][i].cpu().numpy()
    reactive_power = batch['reactive_power'][i].cpu().numpy()
    v_mag_est = batch['v_mag_est'][i].cpu().numpy()
    v_ang_est = batch['v_ang_est'][i].cpu().numpy()
    result_f = batch['result_f'][i].cpu().numpy()
    ...
    for j in range(test_no):
        print(f"the {j} test in the {i} th ac-opf")
        ...
        # --- Validity check ---
        if residual is None or np.isnan(residual) or np.isinf(residual):
            print(f"[ERROR] Invalid residual detected at test #{j}: residual = {residual}")
        ...
        if residual >= case_new.BDD_threshold: # bdd = 58.12403768086803
            TP_RANDOM_ROBUST[posi] += 1
        if test_no ==1:
            break
# %%
path = f'simulation_data/{name}/ac_R'
...
# RESIDUAL_RANDOM = [[],[],[],[],[],[],[]]
# print("ac_opf_no_ac = ", ac_opf_no_ac)
# for i in range(ac_opf_no_ac):
# # for i in range(2):
#     print(f"rand the {i}th ac-opf")
...
#     result_f = batch['result_f'][i].cpu().numpy()
#     COST_pre.append(result['f'])
#     # COST_pre.append(result_f_data[i])
...
# %% save
# os.makedirs(os.path.join(f'final_simulation_data/{name}/ac/Random'), exist_ok=True)
# path = f'final_simulation_data/{name}/ac/Random'
